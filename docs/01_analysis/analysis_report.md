...
- **분석 결론 (Graceful Degradation & IR Type "UNKNOWN")**:
    1. **LLM 자가 수정 (Retry Loop)**: Pydantic 모델 파싱에 실패한 경우, 즉시 실패하지 않고 해당 에러 메시지를 LLM에 다시 주입하여 자기 교정(Self-Correction)을 최대 3회 시도합니다.
    2. **전체 중단 금지**: 3회 재시도에도 불구하고 실패한 경우, 전체 프로세스를 중단하지 않고 `{"type": "UNKNOWN", "raw_text": "...", "reason": "Pydantic Validation Error..."}` 형태의 특별한 IR 객체를 생성하여 파이프라인으로 흘려보냅니다.
