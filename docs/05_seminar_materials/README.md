...
본 폴더는 **"자연어 테스트 스크립트 자동화: Naive RAG에서 Contextual RAG까지의 여정"**이라는 주제로 사내/외 기술 세미나를 진행하기 위한 발표 자료와 스피커 스크립트를 담고 있습니다.
...
| 문서 | 설명 |
| :--- | :--- |
| **[seminar_deck_outline.md](./seminar_deck_outline.md)** | 세미나 프레젠테이션(PPT) 슬라이드 장표별 구성 및 스피커 대본(발표 스크립트) |
| **[demo_scenario_guide.md](./demo_scenario_guide.md)** | 세미나 중 청중의 몰입도를 높이기 위한 시연 시나리오 및 주요 해설 포인트 가이드 |
...
*   **추천 제목**: LLM과 RAG를 실전에 도입하며 배운 것들: 97% 비용 절감과 환각(Hallucination) 제로를 향한 아키텍처 진화기
*   **예상 소요 시간**: 45분 (발표 35분 + Demo 5분 + Q&A 5분)
*   **대상 청중**: LLM/RAG 도입을 고민 중인 개발자, 테스트 자동화 엔지니어, 프로젝트 리더

### 💡 세미나의 3가지 핵심 메시지 (Takeaways)
